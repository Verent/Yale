...
namespace Yale.Parser
{
    ...
    internal class ExpressionTokenizer : Tokenizer
    {
        ...
        private void CreatePatterns()
        {
            ...
            pattern = new TokenPattern((int)Token.ARGUMENT_SEPARATOR, "ARGUMENT_SEPARATOR", PatternType.STRING, ";");
            AddPattern(pattern);

            pattern = new TokenPattern((int)Token.ARRAY_BRACES, "ARRAY_BRACES", PatternType.STRING, "[]");
            AddPattern(pattern);
            ...
            pattern = new TokenPattern((int)Token.REAL, "REAL", PatternType.REGEXP, "\\d*\\.\\d+([e][+-]\\d{1,3})?(d|f|m)?");
            AddPattern(pattern);

            pattern = new TokenPattern((int)Token.STRING_LITERAL, "STRING_LITERAL", PatternType.REGEXP, "\"([^\"\\r\\n\\\\]|\\\\u[0-9a-f]{4}|\\\\[\\\\\"'trn])*\"");
            AddPattern(pattern);

            pattern = new TokenPattern((int)Token.CHAR_LITERAL, "CHAR_LITERAL", PatternType.REGEXP, "'([^'\\r\\n\\\\]|\\\\u[0-9a-f]{4}|\\\\[\\\\\"'trn])'");
            AddPattern(pattern);
            ...
            pattern = new TokenPattern((int)Token.IDENTIFIER, "IDENTIFIER", PatternType.REGEXP, "[a-z_]\\w*");
            AddPattern(pattern);

            pattern = new TokenPattern((int)Token.HEX_LITERAL, "HEX_LITERAL", PatternType.REGEXP, "0x[0-9a-f]+(u|l|ul|lu)?");
            AddPattern(pattern);

            pattern = new TokenPattern((int)Token.NULL_LITERAL, "NULL_LITERAL", PatternType.STRING, "null");
            AddPattern(pattern);

            pattern = new TokenPattern((int)Token.TIMESPAN, "TIMESPAN", PatternType.REGEXP, "##(\\d+\\.)?\\d{2}:\\d{2}(:\\d{2}(\\.\\d{1,7})?)?#");
            AddPattern(pattern);
