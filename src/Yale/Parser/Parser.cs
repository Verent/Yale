...
internal abstract class Parser
{
    ...
    protected virtual Production NewProduction(ProductionPattern pattern) =>
        Analyzer.NewProduction(pattern);
     ...
     */
    internal ProductionPattern GetPattern(TokenId id) => patternIds[id];
     ...
     */
    internal ProductionPattern GetStartPattern() => Patterns[0];
    ...
    internal string? GetTokenDescription(TokenId token) =>
        Tokenizer?.GetPatternDescription(token) ?? string.Empty;
}
