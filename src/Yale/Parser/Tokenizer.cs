...
internal class Tokenizer
{
    ...
    */
     ...
     */
    public int GetCurrentLine() => buffer.LineNumber;
     ...
     */
    public int GetCurrentColumn() => buffer.ColumnNumber;
...
internal abstract class TokenMatcher
{
     ...
     * @param ignoreCase      the character case ignore flag
     */
    public TokenMatcher(bool ignoreCase) => this.ignoreCase = ignoreCase;
...
internal sealed class StringDFAMatcher : TokenMatcher
{
    ...
    {
        var res = automaton.Match(buffer, ignoreCase);
...
internal sealed class RegExpMatcher : TokenMatcher
{
    ...
    public override void Match(ReaderBuffer buffer, TokenMatch match)
    {
        ...
        {
            var length = regExps[i].Match(buffer);
            if (length > 0)
            {
...
internal sealed class GrammaticaRE : REHandler
{
     ...
     *             invalid syntax
     */
    public GrammaticaRE(string regex, bool ignoreCase) => regExp = new RegExp(regex, ignoreCase);
