...
internal sealed class RepeatElement : Element
{
     ...
     */
    public override object Clone() => new RepeatElement((Element)elem.Clone(), min, max, type);
     ...
     * @throws IOException if an I/O error occurred
     */
    private int MatchGreedy(Matcher m, ReaderBuffer buffer, int start, int skip)
    {
     ...
     * @throws IOException if an I/O error occurred
     */
    private int MatchReluctant(Matcher m, ReaderBuffer buffer, int start, int skip)
    {
        // Find all matches
        if (matchStart != start)
        {
     ...
     * @throws IOException if an I/O error occurred
     */
    private int MatchPossessive(Matcher m, ReaderBuffer buffer, int start, int count)
    {
        var length = 0;
        var subLength = 1;
    ...
    )
    {
        ...
        FindMatches(m, buffer, start + subLength, length + subLength, count + 1, 0);
    }
